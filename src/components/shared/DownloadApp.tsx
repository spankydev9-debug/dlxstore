...
export function DownloadApp({ variant = "footer" }: { variant?: "drawer" | "footer" }) {
  ...
  const [dialog, setDialog] = useState<"ios" | "fallback" | null>(null);
  ...
  return (
    <>
      <button
        ...
        className={
          isDrawer
            ? "flex w-full items-center justify-center gap-2 rounded-full bg-primary py-3 text-sm font-semibold text-primary-foreground shadow-sm transition-colors hover:bg-primary/95 focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring"
      ...
      {dialog && (
        ...
        >
          <section className="w-full max-w-sm animate-fade-in rounded-2xl border border-border bg-card p-6 shadow-2xl">
